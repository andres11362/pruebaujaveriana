...
function Carga() { //funcion que cargara dinamicamente la pagina de inicio de departamento
    ...
    var route = 'http://localhost:8000/pais'; //indicamos la ruta de la peticion
...
function Eliminar(btn) {
    ...
    var route = 'http://localhost:8000/pais/'+btn.value; //indicamos la ruta del controlaor en este caso de eliminacion
    var token = $('#token').val();
...
function Mostrar(btn) { //funcion que mostrar los datos en un modal, por medio del valor del boton
    var route = "http://localhost:8000/pais/"+btn.value+"/edit";  //indicamos la ruta del controlaor en este caso de edicion
    $.get(route, function(res){ //se obtiene una respuesta por medio de una peticion la cual seran los datos del modelo departamento
        console.log(res.id); //mostramos los datos por consola
       $('#nombre').val(res.nombre); //se envia a los controles los datos para su posterior actualizacion
       $('#id').val(res.id);
    });
}
...
$('#actualizar').click(function () {
    var value = $('#id').val();
    var dato = $('#nombre').val(); //definimos los valores en los controles
    var route = 'http://localhost:8000/pais/'+value; //indicamos la ruta del controlaor en este caso de eliminacion
